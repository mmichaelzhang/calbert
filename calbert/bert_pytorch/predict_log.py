...
class Predictor():
    ...
    def generate_test(output_dir, file_name, window_size, adaptive_window, seq_len, scale, min_len):
        ...
        # with open(output_dir + file_name, "r") as f:
        data_sentence = json.load(open(output_dir + file_name + '_data_tokens', 'r'))
        for i,line in enumerate(tqdm(data_iter)):
            # print(i, len(data_sentence), data_sentence[i])
            #if idx > 40: break
            log_seq, tim_seq, data_seq = fixed_window(data_sentence = data_sentence[i], line = line, window_size = window_size,
                                            adaptive_window=adaptive_window,
                                            seq_len=seq_len-1, min_len=min_len)
            if len(log_seq) == 0:
                continue
        ...
        data_seqs = np.array(data_seqs, dtype = object)
        ...
        data_sentence = data_seqs[test_sort_index]

        print(f"{file_name} size: {len(log_seqs)}")

        
        
        # data_sentence  = np.array(data_sentence, dtype=object)
        # data_sentence = data_sentence[test_sort_index]
        # print(data_sentence)



        print(log_seqs.shape, tim_seqs.shape, data_sentence.shape)
        return log_seqs, tim_seqs, data_sentence
    ...
    def helper(self, model, output_dir, file_name, vocab, scale=None, error_dict=None):
        ...
        total_dist = []
        output_cls = []
        logkey_test, time_test, data_sentence = self.generate_test(output_dir, file_name, self.window_size, self.adaptive_window, self.seq_len,scale, self.min_len)
        # print(len(logkey_test), len(time_test), len(data_sentence))
        # use 1/10 test data
        ...
        seq_dataset = LogDataset(logkey_test, time_test, data_sentence, vocab, seq_len=self.seq_len,
        ...
        for idx, data in enumerate(data_loader):
            ...
            result = model(data["bert_input"], data["time_input"], data['sentence_input'])
