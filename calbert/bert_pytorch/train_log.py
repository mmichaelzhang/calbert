...
class Trainer():
    ...
    def train(self):
        ...
        logkey_train, logkey_valid, time_train, time_valid, sentence_train, sentence_valid = generate_train_valid(self.output_path,
        ...
        train_dataset = LogDataset(logkey_train,time_train, sentence_train, vocab, seq_len=self.seq_len,
        ...
        valid_dataset = LogDataset(logkey_valid, time_valid, sentence_valid, vocab, seq_len=self.seq_len, on_memory=self.on_memory, mask_ratio=self.mask_ratio)
    ...
    def start_iteration(self, surfix_log):
        ...
        for epoch in range(self.epochs):
            ...
            if self.hypersphere_loss:
                # center = self.calculate_center_raw([self.train_data_loader, self.valid_data_loader])
                center, word_center = self.calculate_center([self.train_data_loader, self.valid_data_loader])
                # center = self.calculate_center([self.train_data_loader])
                self.trainer.word_hyper_center = word_center
    ...
    def calculate_center_raw(self, data_loader_list):
        ...
        with torch.no_grad():
            ...
            for data_loader in data_loader_list:
                ...
                for i, data in data_iter:
                    ...
                    result = self.trainer.model.forward(data["bert_input"], data["time_input"], data["sentence_input"])
    ...
    def calculate_center(self, data_loader_list):
        ...
        with torch.no_grad():
            ...
            outputs_word = 0
            total_samples_word = 0

            for data_loader in data_loader_list:
                totol_length = len(data_loader)
                data_iter = tqdm.tqdm(enumerate(data_loader), total=totol_length)
                # print("Here before to device")
                for i, data in data_iter:
                    data = {key: value.to(self.device) for key, value in data.items()}
                    # print("1",nvidia_smi.nvmlDeviceGetMemoryInfo(handle).used)
                    result = self.trainer.model.forward(data["bert_input"], data["time_input"], data["sentence_input"])
                    # print("2",nvidia_smi.nvmlDeviceGetMemoryInfo(handle).used)
                    cls_output = result["cls_output"]
                    word_cls_output = result['word_cls_outputs']

                    outputs += torch.sum(cls_output.detach().clone(), dim=0)
                    total_samples += cls_output.size(0)

                    outputs_word += torch.sum(word_cls_output.detach().clone(), dim = (0,1))
                    total_samples_word += word_cls_output.size(0) * word_cls_output.size(1)

                    # print(outputs.size(), total_samples, outputs_word.size(), total_samples_word)

                    del data

        center = outputs / total_samples
        word_center = outputs_word / total_samples_word

        return center, word_center
