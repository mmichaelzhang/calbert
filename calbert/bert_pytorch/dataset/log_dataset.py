...
class LogDataset(Dataset):
    def __init__(self, log_corpus, time_corpus, sentence_corpus, vocab, seq_len, corpus_lines=None, encoding="utf-8", on_memory=True, predict_mode=False, mask_ratio=0.15):
        """
    ...
    def __getitem__(self, idx):
        k, t, s = self.log_corpus[idx], self.time_corpus[idx], self.sentence_corpus[idx]
        # print(len(k), len(t), len(s))
        # print(k.size(), t.size(), s.size())

        k_masked, k_label, t_masked, t_label, s_masked, s_label = self.random_item(k, t, s)
        ...
        return k, k_label, t, t_label, s_masked, s_label

    def random_item(self, k, t, s):
        tokens = list(k)
        output_label = []
        ...
        for i, token in enumerate(tokens):
            ...
            for j, word in enumerate(sentence[1:-1]):
                j += 1
                prob = random.random()
                if (prob < self.mask_ratio):
                    if (self.predict_mode):
                        sentence_label[i].append(int(word))
                        sentences[i][j] = 103
                        continue
                    ...
                    if prob < 0.8:
                        ...
                        sentences[i][j] = random.randrange(1000, 30522)
                    else:
                        sentences[i][j] = int(word)
                    sentence_label[i].append(int(word))
                else:
                    sentences[i][j] = int(word)
                    sentence_label[i].append(0)
            sentence_label[i].append(0)
            # print(sentences[i], sentence_label[i])
        ...
        return tokens, output_label, time_intervals, time_label, sentences, sentence_label
    ...
    def collate_fn(self, batch, percentile=100, dynamical_pad=True):
        ...
        for seq in batch:
            ...
            sentence_input = [e[:max_length] for e in seq[4][:seq_len]]
            sentence_label = [e[:max_length] for e in seq[4][:seq_len]]

            padding = [self.vocab.pad_index for _ in range(seq_len - len(bert_input))]
            padding_sen = [[0 for _ in range(max_length)] for _ in range(seq_len - len(bert_input))]
            sentence_input = [[0 for _ in range(max_length)]] + sentence_input
            sentence_label = [[0 for _ in range(max_length)]] + sentence_label
            # print(sentence_input, bert_input)
            bert_input.extend(padding), bert_label.extend(padding), time_input.extend(padding), time_label.extend(
                padding)

            sentence_input.extend(padding_sen)
            sentence_label.extend(padding_sen)
            # print(len(padding), len(padding_sen), len(sentence_input), len(bert_input))

            for i,sentence in enumerate(sentence_input):
                sub_padding = [0 for _ in range(max_length - len(sentence))]
                sentence_input[i].extend(sub_padding)
                sentence_label[i].extend(sub_padding)
            
            

            # print(len(sentence_input), len(sentence_label), len(bert_input))
            # for i, sentence in enumerate(sentence_input):
            #     print(len(sentence), len(sentence_label[i]))
            ...
            output["sentence_input"].append(sentence_input)
            output["sentence_label"].append(sentence_label)
        ...
        # print("Here\n",output["bert_input"].size())
        output["sentence_input"] = torch.tensor(output["sentence_input"], dtype = torch.long)
        output["sentence_label"] = torch.tensor(output["sentence_label"], dtype = torch.long)
