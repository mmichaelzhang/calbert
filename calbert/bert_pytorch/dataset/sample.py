...
def fixed_window(data_sentence, line, window_size, adaptive_window, seq_len=None, min_len=0):
    ...
    if seq_len is not None:
        line = line[:seq_len]
        data_sentence = data_sentence[:seq_len]
    ...
    for i in range(0, len(line), window_size):
        ...
        data_seq.append(data_sentence[i:i + window_size])

    return logkey_seqs, time_seq, data_seq
...
def generate_train_valid(data_path, window_size=20, adaptive_window=True,
    ...
    with open(data_path + 'train', 'r') as f:
        data_iter = f.readlines()

    data_sentence = json.load(open(data_path + 'data_tokens', 'r'))
    ...
    data_seq_pairs = []
    session = 0
    for i,line in enumerate(tqdm(data_iter)):
        # print(i,line)
        # print(data_sentence[i],len(data_sentence[i]))
        if session >= num_session:
            break
        session += 1

        logkeys, times, sentence = fixed_window(data_sentence[i], line, window_size, adaptive_window, seq_len-1, min_len=min_len)
        logkey_seq_pairs += logkeys
        time_seq_pairs += times
        data_seq_pairs += sentence

    logkey_seq_pairs = np.array(logkey_seq_pairs, dtype=object)
    time_seq_pairs = np.array(time_seq_pairs, dtype=object)
    data_sentence  = np.array(data_seq_pairs, dtype = object)

    test_size = int(min(num_session, len(data_sentence)) * valid_size)
    print("before filtering short session")
    print("train size ", int(len(data_sentence) - test_size))
    print("valid size ", int(test_size))
    print("="*40)

    logkey_trainset, logkey_validset, time_trainset, time_validset, sentence_trainset, sentence_validset = train_test_split(logkey_seq_pairs,
                                                                                      time_seq_pairs,
                                                                                      data_sentence,
                                                                                      test_size=test_size,
                                                                                      random_state=1234)
    ...
    sentence_trainset = sentence_trainset[train_sort_index]
    sentence_validset = sentence_validset[valid_sort_index]
    ...
    return logkey_trainset, logkey_validset, time_trainset, time_validset, sentence_trainset, sentence_validset
