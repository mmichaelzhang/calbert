...
from ..model import BERTLog, BERT, BERTLog
...
class BERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
        ...
        self.word_criterion = nn.NLLLoss(ignore_index = 0)
        self.sentence_hyper_criterion = nn.MSELoss()
    ...
    def iteration(self, epoch, data_loader, start_train):
        ...
        for i, data in data_iter:
            ...
            result = self.model.forward(data["bert_input"], data["time_input"], data['sentence_input'])
            ...
            word_loss = self.word_criterion(result["word_output"].transpose(1, -1), data['sentence_label'].transpose(1, -1)).squeeze()
            word_hyper_loss = self.sentence_hyper_criterion(result['word_cls_outputs'].squeeze(), self.word_hyper_center.expand(result['word_cls_outputs'].shape[0], result['word_cls_outputs'].shape[1], -1).squeeze())
            ...
            if self.hypersphere_loss:
                ...
                # print(result["cls_output"].squeeze().size(), self.hyper_center.expand(data["bert_input"].shape[0], -1).size())
                hyper_loss = self.hyper_criterion(result["cls_output"].squeeze(), self.hyper_center.expand(data["bert_input"].shape[0], -1).squeeze())
            ...
            loss = 0.1 * loss + word_loss + 0.1 * word_hyper_loss
